...
async function main() {
  ...
  for (const s of SAMPLES) {
    ...
    // 鐵證 = 主角名缺席 + 首尾主題漂移。
    // （單看 drift 會誤報第1章 —— 其開頭是「角色卡數值面板」用詞特殊；主角名次數才是乾淨判別子。）
    const polluted = heroHits === 0 && drift;
    console.log(
      polluted
        ? `   🚩 內文污染：主角「${HERO}」全章 0 次 + 首尾主題漂移 → 後段為其他小說填充句`
        : heroHits > 0
          ? `   ✅ 內文乾淨：主角「${HERO}」現身 ${heroHits} 次，正文連貫`
          : `   ⚠️ 主角未出現但首尾連貫（可能為配角視角章，需人工複看）`,
    );
  }
  console.log(
    "\n" +
      "=".repeat(60) +
      "\n結論：第1章（免費試讀）內文乾淨，但連載段（689 章後，正是我們缺的部分）主角名消失、" +
      "後段塞滿他書填充句 —— xszj 對 VIP 連載段做了反爬閹割，【內文不可用】。",
  );
}
